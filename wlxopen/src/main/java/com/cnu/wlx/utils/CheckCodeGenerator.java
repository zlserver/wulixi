...
public class CheckCodeGenerator {
	...
	public CheckCodeGenerator(int hight, int width) {
		super();
		this.hight = hight;
		this.width = width;
		this.modle = 2;
	...
	private String drawString(Graphics2D g,int modle) {
		...
		//字符大小，以及字体
		g.setFont(new Font("宋体", Font.BOLD, this.hight-4));
		// 常见的中文 UTF-8编码
		...
		Random r = new Random();
		//字符开始画的（x,y）坐标，x是距离图左边距离，y是要画的点距离上面的距离
		int y = this.hight/2+(this.hight/2)/2;
		int x = 10;
		for (int i = 0; i < 4; i++) {
			int d = r.nextInt() % 30;
			//画的字符
